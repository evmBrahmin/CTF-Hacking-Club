...
contract FlatLaunchpeg is BaseLaunchpegNFT {
    ...
    {
        // @audit likely something here to check if the user has already minted
        if (numberMinted(msg.sender) + _quantity > maxPerAddressDuringMint) {
            revert Launchpeg__CanNotMintThisMany();
        }
        // @audit TokenID.current + _quantity must be <= collectionSize
        // Last call must be _quantity = collectionSize - totalSupply
